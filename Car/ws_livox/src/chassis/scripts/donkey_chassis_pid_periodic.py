...
class MotorController:
    def __init__(self):
        ...
        rospy.logwarn(f"init done, waiting for /Odometry topic...")
        rospy.spin()
    ...
    def set_deg(self, deg: float):
        if deg < 0:
            pulse = np.interp(deg, [-15.6, 0], right_range)
        elif deg > 0:
            pulse = np.interp(deg, [0, 15.6], left_range)
        else:
    ...
    def odometry_callback(self, msg):
        if not self.vpid.auto_mode:
            ...
            rospy.Timer(rospy.Duration(0.1), update_velocity)
